...
import static org.jooq.sources.Tables.ACTOR;
import static org.jooq.sources.tables.Customer.CUSTOMER;
import static org.jooq.sources.tables.Film.FILM;
import static org.jooq.sources.tables.FilmActor.FILM_ACTOR;
import static org.jooq.sources.tables.FilmCategory.FILM_CATEGORY;

/**
 * D'après :
 * https://github.com/jOOQ/demo/blob/main/jOOQ-demo-pro/jOOQ-demo-java/src/test/java/org/jooq/demo/java/Demo01Querying.java
 */
public class Demo01Querying extends Demo {

    @Test
    public void fetchDual() {
        title("A simple SELECT 1 FROM DUAL (if needed)");
        println(ctx.selectOne().fetch()); // select 1 as "one"
    ...
    public void consumeRecordsForEach() {
        ...
        //for (var r : ctx
        for (Record2<String, String> r : ctx
                .select(ACTOR.FIRST_NAME, ACTOR.LAST_NAME)
                .from(ACTOR)
                .where(ACTOR.ACTOR_ID.lt(5))
        ) {
            println("Actor: %s %s".formatted(r.value1(), r.value2()));
        }

        title("\nThis also means we can call Iterable::forEach on it");
    ...
    /**@Test public void nestingToManyRelationshipsAsMaps() {
